	...
	public class MMCinemachineZoom : MonoBehaviour
	{
        ...
        protected virtual void Update()
        {
            ...
            if (_elapsedTime <= _transitionDuration)
            {
                ...
                if (_virtualCamera.m_Lens.Orthographic)  // Check if the camera is orthographic
                {
                    _virtualCamera.m_Lens.OrthographicSize = Mathf.LerpUnclamped(_startOrthographicSize, _targetOrthographicSize, ZoomTween.Evaluate(t));  // Adjust orthographic size
                }
                else
                {
                    _virtualCamera.m_Lens.FieldOfView = Mathf.LerpUnclamped(_startFieldOfView, _targetFieldOfView, ZoomTween.Evaluate(t));  // Adjust field of view
                }
            }
            else
            {
        ...
        public virtual void Zoom(MMCameraZoomModes mode, float newSize, float transitionDuration, float duration, bool useUnscaledTime, bool relative = false, MMTweenType tweenType = null)
        {
            if (_zooming)
            {
                return;
            ...
            if (_virtualCamera.m_Lens.Orthographic)  // Handle orthographic mode
            {
                _startOrthographicSize = _virtualCamera.m_Lens.OrthographicSize;
            }
            else  // Handle perspective mode
            {
            ...
            switch (mode)
            {
                ...
                case MMCameraZoomModes.Set:
                    if (_virtualCamera.m_Lens.Orthographic)  // Adjust orthographic size
                    {
                        _targetOrthographicSize = relative ? newSize + _initialOrthographicSize : newSize;
                    }
                    else  // Adjust field of view
                    {
                        _targetFieldOfView = relative ? newSize + _initialFieldOfView : newSize;
                    }
                    break;
